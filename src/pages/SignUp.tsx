...
export function SignUp() {
    ...
    return (
      <AuthLayout>
        <div className="flex flex-col gap-4">
          {/* Accent check mark */}
          <div className="w-10 h-10 rounded-xl bg-accent-100 dark:bg-accent-700/20 flex items-center justify-center">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-accent-600 dark:text-accent-500">
              <polyline points="20 6 9 17 4 12" />
            </svg>
          </div>
          <div>
            <h1 className="display-tight text-2xl text-ink-950 dark:text-white mb-1">Check your email</h1>
            <p className="text-[13px] text-ink-500 leading-relaxed">
              We sent a verification link to <span className="text-ink-950 dark:text-white font-medium">{email}</span>.
              Click it to activate your account — it expires in 24 hours.
            </p>
          </div>
          <p className="text-[12px] text-ink-400">
            Already verified?{' '}
            <Link to="/sign-in" className="text-ink-950 dark:text-white underline underline-offset-2 decoration-ink-300 dark:decoration-ink-700 hover:decoration-ink-950 dark:hover:decoration-white transition-all">
              Sign in
            </Link>
          </p>
        </div>
  ...
  return (
    <AuthLayout>
      <h1 className="display-tight text-2xl text-ink-950 dark:text-white mb-1">Create account</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <label className="block">
          <span className="text-[11px] font-medium text-ink-500 mb-1.5 block tracking-widest uppercase">Email</span>
        ...
        <div className="block">
          <label className="block">
            <span className="text-[11px] font-medium text-ink-500 mb-1.5 block tracking-widest uppercase">Password</span>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
              autoComplete="new-password" required minLength={8} placeholder="••••••••"
              className="w-full h-10 px-3 rounded-xl border border-ink-200 dark:border-ink-800 bg-ink-50 dark:bg-ink-900 text-ink-950 dark:text-white text-[13px] placeholder:text-ink-300 dark:placeholder:text-ink-700 focus:outline-none focus:ring-2 focus:ring-ink-950/10 dark:focus:ring-white/10 focus:border-ink-400 dark:focus:border-ink-600 transition-all" />
          </label>
          <p className="text-[11px] text-ink-400 mt-1.5">At least 8 characters.</p>
        </div>
